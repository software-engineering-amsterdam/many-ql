...
class QLTypeCheckerSpec extends Specification with ExceptionMatchers {
  ...
  "statement" should {
    "add variable + type to environment, if statement is boolean question" in {
      check(BooleanQuestion(Variable("X"), "label"), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> BooleanType())))
    }

    "add variable + type to environment, if statement is number question" in {
      check(NumberQuestion(Variable("X"), "label"), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> NumberType())))
    }

    "add variable + type to environment, if statement is string question" in {
      check(StringQuestion(Variable("X"), "label"), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> StringType())))
    }

    "add variable + type to environment, if statement is computed boolean question with valid expression" in {
      check(ComputedBooleanQuestion(Variable("X"), "label", BooleanLiteral(true)), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> BooleanType())))
    }

    "throw exception, if statement is computed boolean question with invalid expression" in {
      check(ComputedBooleanQuestion(Variable("X"), "label", NumberLiteral(1)), new Environment()) must throwA[RuntimeException]
    }

    "add variable + type to environment, if statement is computed number question with valid expression" in {
      check(ComputedNumberQuestion(Variable("X"), "label", NumberLiteral(1)), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> NumberType())))
    }

    "throw exception, if statement is computed number question with invalid expression" in {
      check(ComputedNumberQuestion(Variable("X"), "label", BooleanLiteral(false)), new Environment()) must throwA[RuntimeException]
    }

    "add variable + type to environment, if statement is computed string question with valid expression" in {
      check(ComputedStringQuestion(Variable("X"), "label", StringLiteral("")), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> StringType())))
    }

    "throw exception, if statement is computed string question with invalid expression" in {
      check(ComputedStringQuestion(Variable("X"), "label", NumberLiteral(1)), new Environment()) must throwA[RuntimeException]
    }

    "return empty environment, if valid boolean condition" in {
      check(IfStatement(BooleanLiteral(true), BooleanQuestion(Variable("X"), "label"), None), new Environment()) must beEqualTo(new Environment())
    }

    "throw exception, if invalid boolean condition" in {
      check(IfStatement(NumberLiteral(0), BooleanQuestion(Variable("X"), "label"), None), new Environment()) must throwA[RuntimeException]
    }

    "return empty environment, if valid boolean condition" in {
      check(IfStatement(BooleanLiteral(true), BooleanQuestion(Variable("X"), "label"), Some(BooleanQuestion(Variable("X"), "label"))), new Environment()) must beEqualTo(new Environment())
    }

    "throw exception, if invalid boolean condition" in {
      check(IfStatement(NumberLiteral(0), BooleanQuestion(Variable("X"), "label"), Some(BooleanQuestion(Variable("X"), "label"))), new Environment()) must throwA[RuntimeException]
    }

    "add variables + types to environment" in {
      check(Sequence(List(BooleanQuestion(Variable("X"), "label"), NumberQuestion(Variable("Y"), "label"), StringQuestion(Variable("Z"), "label"))), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> BooleanType(), "Y" -> NumberType(), "Z" -> StringType())))
    }
    
    "add variable + type to environment " in {
      check(Form("Form1", BooleanQuestion(Variable("X"), "label")), new Environment()) must beEqualTo(new Environment(typeOfFields = Map("X" -> BooleanType())))
    }
  }
  ...
  "type checker" should {
    "detect duplicate question declarations" in {
      check(BooleanQuestion(Variable("X"), "label"), new Environment(typeOfFields = Map("X" -> BooleanType()))) must throwA[RuntimeException]
    }
  }
}
