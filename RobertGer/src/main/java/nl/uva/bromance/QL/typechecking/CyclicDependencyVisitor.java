...
public class CyclicDependencyVisitor implements QLNodeVisitorInterface{
    ...
    public CyclicDependencyVisitor(QLNode node)
    {
        this.node = node;
    }
        ...
        for (VariableList varList : variableLookupList)
        {
            for (String var : varList.getVars())
            {
                if (var.equals(question.getIdentifier()))
                    exceptions.add(new TypeCheckingError("Question @ line "+question.getLineNumber()+" has cyclic dependency @ line "+varList.getLineNumber(), TypeCheckingError.Type.ERROR));
            }
        }
    }
        ...
        for (VariableList varList : variableLookupList)
        {
            for (String var : varList.getVars())
            {
                if (var.equals(calc.getIdentifier()))
                    exceptions.add(new TypeCheckingError("Calculation @ line "+calc.getLineNumber()+" has cyclic dependency @ line "+varList.getLineNumber(), TypeCheckingError.Type.ERROR));
            }
        }
    }
